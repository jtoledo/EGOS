...
$valida_compra = validar_campos_obligatorios(array($id_compra));	
	...
	if($band==1 and $form==1 and $band_almacen==1)
	{
			...
			$fila=get_nota_arreglo_individual($idcliente,$_SESSION['mov_sucursal'],$_SESSION['cosecha_sel']);	
			$modif=1;
	...
	{
	   $fila=get_nota_arreglo($id_compra,$_SESSION['mov_sucursal'],$_SESSION['cosecha_sel']);  //cuando no hay una consulta determinada se consulta la primera solicitud
	   if($fila!=0)
			{
			$modif=1;
$id_compra=$fila["id_compra"];
