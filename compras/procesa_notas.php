...
if($enviar=="GUARDAR")
	{
				//validaciones				
				if($folio!='') {				
						$sql="select count(*)::integer as folioband from co_compras where folio='$folio' and serie='$nserie' and id_almacen='$id_almacen' and id_periodo='$id_periodo'";

						$fila =pg_fetch_assoc(pg_query($con,$sql));

						if($fila["folioband"]>=1) {
							/*encontro folio y se sale*/
							echo 1;
							exit;				
						}
						/*						
						//validar si el folio es valido 20 hacia delante y 50 hacia atras
						$sql="select case when trim('$nserie')='' then folio_inicial::integer else case when trim('$nserie')='A' 
								then folio_sa else case when trim('$nserie')='B' then folio_sb else case when trim('$nserie')='C' 
								then folio_sc else case when trim('$nserie')='D' then folio_sd else folio_consigna end
								end end end end  as folio_actual
								from al_almacenes where id_sucursal=$id_almacen";
						$fila =pg_fetch_assoc(pg_query($con,$sql));
						$folio_dig=(int)$folio; 
						
						if($folio_dig>=$fila["folio_actual"]) {						
								if($fila["folio_actual"]+15<$folio_dig) {
										//valida folio
										echo 2;
										exit;				
								}
						}else {						
								if($fila["folio_actual"]-60>$folio_dig) {
										echo 2;
										exit;								
								}
						} */
            ...
            { 
						$resultado=$row[0];
						$err=substr($resultado,1,(strlen($resultado)));
						$err =substr($err, 0,-1);
						$err=trim($err);
						if($err==-3) {
							echo 1;
...
include("nota_m.php");
	...
	$sql= "SET datestyle TO postgres, dmy; select co_modificar_nentrada_egos('$idproductor','$id_almacen','$idcliente','$id_catalogo','$produccion','$fecha_nota','$vehiculo','$henequen','$yute','$bolsa','$kgs_brutos','$tonga','$resto_alimentos','$desechos_human','$olores_desa','$otros_organicos','$manchas_comb','$vehiculo_sucio','$olor_detergente','$otros_inorganicos','$precio_kilo','$rendimiento','$mancha','$humedad','$cerezo','$criba','$resultado_analisis','$constancia_cmc','$total_kgs_netos','$forma_pago',$no_cheque,$banco_cheque,'$costalera','$estado_costalera','$observaciones','$id_usuario','$total_kgs_brutos','$total_kgs_netos','$total_kgs_tara','$subtotal','$id_compra',$id_servicio,'$id_periodo',$retencion_p,$retencion_peso,$miembro_c)";

				$query = pg_query($con,$sql);	
				
				//registra logs
				$usuario=$_SESSION["nombre_u"];
				$sql="select registrar_logs(1,'{$id_compra}','co_compras','M','{$usuario}',
				'Modificacion de nota de entrada folio :'||(select folio from co_compras where id_compra={$id_compra}));";
				$querytmp = pg_query($con,$sql);
		...
		echo "<div align='center'><strong><font color='#003300'>La nota de entrada fue modificada correctamente</font></strong></div>";
		$fila=get_nota_arreglo($id_compra);  //se recupera la consulta de la nota de entrada
