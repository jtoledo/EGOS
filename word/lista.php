...
$d=mysql_num_rows($r);
if ($d>=1){
	echo(" <form action=\"confirma2.php\" method=\"post\">    ");
	echo (" <table border=1> ");
	echo ("<tr>");
	echo ("<td> Clave </td>");
	echo ("<td> Nombre </td>");
	...
	while($registro=mysql_fetch_row($r)){
		...
		echo ("<td> <input type=\"checkbox\" name=\"s$n\" value=\"$registro[0]\" </td>");
...
echo (" </table> ");
echo("<input type=\"submit\" value=\"Eliminar\"> <input type=\"button\" value=\"Cancelar\" onclick=\"history.back()\">");
echo("</form>");
}
else{
