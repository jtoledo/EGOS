...
/*FIN DEL TRATAMIENTO DE LA INFORMACION*/
	...
	if($enviar=="MODIFICAR")
	{
				...
				//validar si el registro ya existe en la tabla de mermas
				$sqlvalid="select count(*)::integer as nregistro from co_compras_merma where id_compra='$id_compra'";
				$RsQuery = pg_fetch_array(pg_query($con,$sqlvalid));			

				if($RsQuery['nregistro']<=0) {				
						//inserta datos				
						$sql= "SET datestyle TO postgres, dmy; select co_genera_nentrada_egos_merma('$id_compra','$idproductor','$id_almacen','$idcliente','$id_catalogo','$produccion','$fecha_nota','$vehiculo','$henequen','$yute','$bolsa','$kgs_brutos','$tonga','$resto_alimentos','$desechos_human','$olores_desa','$otros_organicos','$manchas_comb','$vehiculo_sucio','$olor_detergente','$otros_inorganicos','$precio_kilo','$rendimiento','$mancha','$humedad','$cerezo','$criba','$resultado_analisis','$constancia_cmc','$total_kgs_netos','$forma_pago',$no_cheque,$banco_cheque,'$costalera','$estado_costalera','$observaciones','$id_usuario','$total_kgs_brutos','$total_kgs_netos','$total_kgs_tara','$subtotal',$id_servicio,'$id_periodo',$retencion_p,$retencion_peso,$miembro_c,'$nserie','$folio')";
				}else {

						//modifica datos
						$sql= "SET datestyle TO postgres, dmy; select co_modificar_nentrada_egos_merma('$idproductor','$id_almacen','$idcliente','$id_catalogo','$produccion','$fecha_nota','$vehiculo','$henequen','$yute','$bolsa','$kgs_brutos','$tonga','$resto_alimentos','$desechos_human','$olores_desa','$otros_organicos','$manchas_comb','$vehiculo_sucio','$olor_detergente','$otros_inorganicos','$precio_kilo','$rendimiento','$mancha','$humedad','$cerezo','$criba','$resultado_analisis','$constancia_cmc','$total_kgs_netos','$forma_pago',$no_cheque,$banco_cheque,'$costalera','$estado_costalera','$observaciones','$id_usuario','$total_kgs_brutos','$total_kgs_netos','$total_kgs_tara','$subtotal','$id_compra',$id_servicio,'$id_periodo',$retencion_p,$retencion_peso,$miembro_c)";
				}
				...
				$sql="select registrar_logs(1,'{$id_compra}','co_compras_merma','M','{$usuario}',
				'Registro de merma folio :'||(select folio from co_compras_merma where id_compra={$id_compra}));";
		...
		echo "<div align='center'><strong><font color='#003300'>La Merma fue registrada correctamente</font></strong></div>";
			
		$fila=get_nota_arreglo_merma($id_compra);  //se recupera la consulta de la nota de entrada
