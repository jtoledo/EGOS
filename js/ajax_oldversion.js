...
//			divFormulario.innerHTML = ajax.responseText
			...
			//de-serializar
			preciox_kg=ajax.responseText.substring(0,ajax.responseText.indexOf("|"));
			retencion_p=ajax.responseText.substring(ajax.responseText.indexOf("|")+1,ajax.responseText.length);			
...
ajax.send("id_catalogo="+id_catalogo+"&form="+form)
...
if (confirm("Esta completamente seguro de guardar servicio ?"))
{
	      ...
	      if(typeof(document.captura.idcliente.value)!='undefined')
			{			
				if (document.captura.idcliente.value.length==0)
				{ 
				 	alert("Cliente no puede ser vacio"); 
