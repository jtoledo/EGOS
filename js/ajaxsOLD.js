...
function clientes_alta_egos(form_m){
	...
	/*if (document.captura.clv_fira.value.length==0)
	{ 
      	 alert("Clave fira no puede ser vacio"); 
      	 document.captura.clv_fira.focus(); 
      	 return 0; 
	}
	
	if (document.captura.clv_ha.value.length==0)
	{ 
      	 alert("Clave ha no puede ser vacio"); 
      	 document.captura.clv_ha.focus(); 
      	 return 0; 
	}
	
	if (document.captura.id_cuenta.value.length==0)
	{ 
      	 alert("Numero de cuenta no puede ser vacio"); 
...
alert("Muchas gracias la informacion ha sido enviada");
	...
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			...
			if(ajax.responseText!=0  && ajax.responseText!=1 && ajax.responseText!=2 )
			{
				divFormulario.innerHTML = ajax.responseText
...
function valida_modarticulos(form_m) {
   ...
   var control=0;
   for (var i=0;i<document.captura.precio.length;i++)
	{ 
		if(document.captura.precio[i].value=='')
		{
			control=1;
		}
		if (document.captura.precio[i].value<=0) {
			control=1;	
		}
	}
	if (control==1)
	{ 
      	 alert("Verifique que los campo de precio no se encuentren vacios o Menor a $0.00"); 
      	 document.captura.precio[0].focus(); 
      	 return 0; 
	}
	
	clv_precio='&';	
	for (var i=0;i<document.captura.clv_precio.length;i++)
	{ 
		if(document.captura.clv_precio.length-1==i)		
		{
			clv_precio=clv_precio+"precio"+document.captura.clv_precio[i].value+'='+document.captura.precio[i].value;		
		}else{
			clv_precio=clv_precio+"precio"+document.captura.clv_precio[i].value+'='+document.captura.precio[i].value+'&';
		}
	}
	...
	ajax.send("idarticulos="+idarticulo+clv_precio);
