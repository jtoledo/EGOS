...
if($enviar=="GUARDAR" /*and $band==1*/)
	{
		...
		$sql= "SET datestyle TO postgres, dmy; select nuevo_garantiash('$idcliente','$descripcion','$valor','$fvaluacion','$registro','$libro','$tomo','$seccion','$volumen','$superficie','$fregistro','$antecedentes','$id_usuario')";
				...
				'Alta de garantias hipotecarias del cliente:'||(select nombre||' '||ap_paterno||' '||ap_materno from cc_clientes where idcliente={$idcliente}));";
...
if($consulta>0)
	   ...
	   $id_query = pg_query($con,$consulta);
				...
				'Modificacion de garantias hipotecarias del cliente:'||(select nombre||' '||ap_paterno||' '||ap_materno from cc_clientes where idcliente={$idcliente}));";
				$querytmp = pg_query($con,$sql);
